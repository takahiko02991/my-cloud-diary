...
st.divider()
st.subheader("📅 日記カレンダー")

def show_diary_calendar():
    ...
    try:
        ...
        # 5. カレンダーのイベントをクリックした時の処理
        if state.get("eventClick"):
            event_id = int(state["eventClick"]["event"]["id"])
            # クリックされた日記の詳細を検索
            selected_entry = next((e for e in diary_entries if e["id"] == event_id), None)
            
            if selected_entry:
                st.write("---")
                st.markdown(f"### 📖 {selected_entry['title']}")
                st.info(f"📅 記録日: {selected_entry['created_at']}")
                st.write(selected_entry['content'])
                if selected_entry.get('temp_kitakyushu'):
                    st.caption(f"🌡️ 北九州: {selected_entry['temp_kitakyushu']}℃ / 西都: {selected_entry['temp_saito']}℃")

    except Exception as e:
        st.error(f"日記の読み込みに失敗しました: {e}")
        st.info("まずはSupabaseに 'diaries' テーブルを作成して、何か日記を保存してみてね！")
